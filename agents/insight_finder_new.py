...
class InsightFinderAgent:
    ...
    def process_data(self, data_package: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Extract text data from all relevant columns
        self.text_data = data_package.get("text_columns", {})
        
        # If specific columns are missing, log warning
        required_columns = ["close_notes", "work_notes", "resolution"]
        ...
        # Find patterns in the data combining all text fields
    ...
    def _identify_patterns(self) -> List[Dict[str, Any]]:
        """
        Identify patterns in the text data combining close notes, work notes, and resolution.
        ...
        # Collect all available text fields
        all_text_fields = {}
        for field in ["close_notes", "work_notes", "resolution"]:
            if field in self.text_data and self.text_data[field]:
                all_text_fields[field] = self.text_data[field]
        
        if not all_text_fields:
            self.logger.warning("No text fields available for pattern identification")
            return self._create_placeholder_patterns()
        
        # Create a combined analysis text for each ticket
        combined_texts = []
        for i in range(len(next(iter(all_text_fields.values())))):  # Get length from first available field
            ticket_text = []
            for field, texts in all_text_fields.items():
                if i < len(texts) and texts[i]:
                    ticket_text.append(f"{field}: {texts[i]}")
            combined_texts.append(" ".join(ticket_text))
        
        try:
            # Try the advanced NLP approach with the combined texts
            self.logger.info("Attempting pattern identification with advanced NLP on combined text fields")
            
            # Preprocess the text
            processed_texts = self._preprocess_text(combined_texts)
            
            # Remove empty strings
            non_empty_texts = [text for text in processed_texts if text.strip()]
            
            if len(non_empty_texts) > 10:  # Only analyze if we have enough data
                # Placeholder for TF-IDF and clustering logic
                # This is where you would add your actual pattern detection logic
                # For example:
                from sklearn.feature_extraction.text import TfidfVectorizer
                from sklearn.cluster import KMeans
                ...
                # Perform clustering
                n_clusters = min(5, len(non_empty_texts) // 2)  # Adaptive number of clusters
                kmeans = KMeans(n_clusters=n_clusters, random_state=42)
                cluster_labels = kmeans.fit_predict(tfidf_matrix)
                
                # Create patterns based on clustering results
                for cluster_id in range(n_clusters):
                    ...
                    patterns.append({
                        ...
                        'centroid': vectorizer.inverse_transform(kmeans.cluster_centers_[cluster_id])[0]
                    })
        
        except Exception as e:
            self.logger.error(f"Error in advanced NLP pattern detection: {str(e)}")
            # Fall back to simpler pattern detection if advanced method fails
            patterns = self._fallback_pattern_detection(combined_texts)
        
        # If no patterns were found, create placeholder patterns
        if not patterns:
            patterns = self._create_placeholder_patterns()
    ...
    def _identify_patterns(self) -> List[Dict[str, Any]]:
        """
        Identify patterns in the text data with improved keyword selection.
        ...
        # Custom stopwords to filter out in addition to standard stopwords
        custom_stopwords = {
            'coopervision', 'please', 'group', 'appropriate', 'com', 'resolution', 
            'transfer', 'team', 'sla', 'ticket', 'system', 'email', 'user', 'issue',
            'request', 'access', 'service', 'support', 'thanks', 'thank', 'hello',
            'regards', 'hi', 'dear', 'morning', 'afternoon', 'evening', 'day',
            'assigned', 'assignment', 'status', 'updated', 'update', 'closed'
        }
        
        # Extended stop words
        extended_stop_words = self.stop_words.union(custom_stopwords)
        ...
        if "close_notes" in self.text_data and self.text_data["close_notes"]:
            ...
            try:
                ...
                # Function to preprocess with custom stopwords
                def custom_preprocessor(text_list):
                    processed = []
                    for text in text_list:
                        if not text or text == 'nan':
                            processed.append('')
                        ...
                        # Filter using extended stopwords
                        filtered_tokens = [
                            token for token in tokens 
                            if token not in extended_stop_words 
                            and len(token) > 3  # Longer minimum length
                        ]
                ...
                processed_notes = custom_preprocessor(notes)
                ...
                if len(non_empty_notes) > 10:  # Only analyze if we have enough data
                    # Use TF-IDF with more aggressive filtering
                    ...
                    for i in range(num_clusters):
                        ...
                        # Get top terms for this cluster with additional filtering
                        cluster_center = cluster_centers[i]
                        top_term_indices = cluster_center.argsort()[-20:][::-1]  # Get more candidates
                        
                        # Filter the top terms to exclude any stopwords that might have slipped through
                        top_terms = []
                        for idx in top_term_indices:
                            term = feature_names[idx]
                            if term not in extended_stop_words and len(term) > 3:
                                top_terms.append(term)
                        
                        # Truncate to the first 10 meaningful terms
                        top_terms = top_terms[:10]
                        
                        # If still no meaningful terms, try bigram/trigram analysis
                        if not top_terms:
                            # Analyze specific phrases in this cluster's notes
                            all_cluster_text = " ".join([notes[idx].lower() for idx in cluster_indices])
                            phrases = re.findall(r'\b(\w+\s+\w+(?:\s+\w+)?)\b', all_cluster_text)
                            phrase_counter = Counter(phrases)
                            
                            # Get phrases not containing stopwords
                            meaningful_phrases = []
                            for phrase, count in phrase_counter.most_common(20):
                                words = phrase.split()
                                if not any(word in extended_stop_words for word in words) and count > 1:
                                    meaningful_phrases.append(phrase)
                            
                            top_terms = meaningful_phrases[:5]  # Take top 5 meaningful phrases
                        
                        # If still no terms, use simple word frequency as last resort
                        if not top_terms:
                            all_words = " ".join([notes[idx].lower() for idx in cluster_indices]).split()
                            word_counter = Counter([w for w in all_words if w not in extended_stop_words and len(w) > 3])
                            top_terms = [word for word, _ in word_counter.most_common(5)]
                        
                        # Create pattern dictionary
                        pattern = {
                            "id": f"pattern_{i+1}",
                            "keywords": top_terms if top_terms else ["technical issue"],  # Fallback if still empty
    ...
    def _generate_automation_suggestions(self) -> List[Dict[str, Any]]:
        ...
        Ensures suggestions are never blank with meaningful fallbacks.
        ...
        prompt_template = PromptTemplate(
            ...
            1. The root cause of the problem (what's causing these tickets), can be found in Work notes, short description 
            2. A specific suggested automation solution. Check the Close notes or resolution notes to identify the resolution and suggest accordingly
            3. Why this is the best solution
            
            IMPORTANT: If you can't determine a clear automation solution, do not leave your response blank. Instead, provide one of these responses:
            - If the pattern suggests expired tokens: "Token expiration issues require an automated renewal system that refreshes authentication before timeout."
            - If no clear pattern exists: "No specific automation identified for this pattern, recommend further data collection."
        ...
        for pattern in self.patterns:
            ...
            try:
                ...
                if not root_cause or len(root_cause) < 10:
                    root_cause = "Unable to clearly identify the root cause from available data"
                    
                if not solution or len(solution) < 10:
                    if any("token" in kw.lower() for kw in pattern["keywords"]):
                        solution = "Token expiration issues require an automated renewal system that refreshes authentication before timeout."
                    else:
                        solution = "No specific automation identified for this pattern, recommend further data collection."
                
                if not justification or len(justification) < 10:
                    justification = "This approach addresses the immediate needs while more comprehensive data is gathered."
                ...
                # Add a fallback suggestion even when exceptions occur
                suggestions.append({
                    "id": f"suggestion_{pattern['id']}",
                    "pattern_id": pattern['id'],
                    "keywords": pattern['keywords'],
                    "problem_root_cause": "Analysis error occurred when processing this pattern",
                    "suggested_solution": "No specific automation identified for this pattern, recommend further data collection.",
                    "justification": "Technical limitations prevented complete analysis of this pattern.",
                    "frequency": pattern['frequency'],
                    "percentage": pattern['percentage']
                })
